...
<template>
  <div class="relative">
    ...
    <div v-if="open" class="absolute right-0 z-50 mt-2 w-56 rounded-xl shadow-2xl bg-white py-2">
      <!-- Normal menu -->
      <button
        v-for="item in menus"
        :key="item.label"
      ...
      <div class="grid grid-cols-2 gap-2 px-3">
        <button
          ...
          :key="item.label"
          :class="[
            'flex items-center justify-center gap-1 rounded-lg py-2 hover:bg-slate-100',
            item.class,
          ]"
        >
          <component :is="item.icon" class="h-4 w-4" />
