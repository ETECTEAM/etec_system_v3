        ...
        <div class="w-full">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-slate-900 dark:text-gray-100 tracking-tight">{{ $t('Tracks') }}</h1>
                        <p class="text-sm text-slate-500 dark:text-gray-400 mt-0.5">{{ $t('Manage all course tracks in one place') }}</p>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-2 gap-3 mb-6">
                <div class="bg-white dark:bg-gray-900 rounded-xl border border-slate-200 dark:border-gray-800 px-4 py-3.5">
                    ...
                    <p class="text-xl font-bold text-slate-900 dark:text-gray-100 mt-1">{{ tracks.length }}</p>
                </div>
                <div class="bg-white dark:bg-gray-900 rounded-xl border border-slate-200 dark:border-gray-800 px-4 py-3.5">
                ...
                <div class="w-full sm:max-w-sm">
                    <div class="relative">
                        <input v-model="search" type="text" :placeholder="$t('Search tracks...')"
                            class="w-full rounded-xl border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-800 pl-9 pr-4 py-2.5 text-sm text-slate-700 dark:text-gray-200 placeholder:text-slate-400 dark:placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-500/20 focus:border-transparent transition"
                ...
                <div class="overflow-x-auto">
                    <table class="w-full min-w-[640px]">
                        <thead class="bg-slate-50/80 dark:bg-gray-800/60 border-b border-slate-200 dark:border-gray-800">
                            <tr>
                                <th class="text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-gray-400 px-4 py-3.5 w-12">
                                    {{ $t('No') }}</th>
                                <th class="text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-gray-400 px-4 py-3.5">
                                    {{ $t('Name') }}</th>
                                <th class="text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-gray-400 px-4 py-3.5">
                                    {{ $t('Sub Category') }}</th>
                                <th class="text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-gray-400 px-4 py-3.5">
                                ...
                                <th class="text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-gray-400 px-4 py-3.5">
                                    {{ $t('Status') }}</th>
                                <th class="text-right text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-gray-400 px-4 py-3.5">
                                    {{ $t('Actions') }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100 dark:divide-gray-800">
                            <tr v-for="(track, index) in paginatedTracks" :key="track.id"
                                class="hover:bg-slate-50/60 dark:hover:bg-gray-800/50 transition-colors">
                                <td class="px-4 py-3.5 text-sm text-slate-500 dark:text-gray-400">
                                ...
                                <td class="px-4 py-3.5">
                                    <span class="inline-flex items-center gap-1.5 text-xs font-medium"
                                        :class="track.status === 'active' ? 'text-emerald-700 dark:text-emerald-400' : 'text-rose-700 dark:text-rose-400'">
