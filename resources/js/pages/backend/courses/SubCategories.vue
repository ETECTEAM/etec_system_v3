    ...
    <DashboardLayout>
        <div class="p-4 sm:p-6">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900">Sub Categories</h1>
                    <p class="text-sm text-slate-500">Manage course sub categories</p>
                </div>
                <Link
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
                ...
                <span class="text-sm text-slate-500 whitespace-nowrap">
                    Showing <strong class="text-slate-700">{{ paginatedSubCategories.length }}</strong> of 
                    <strong class="text-slate-700">{{ filteredSubCategories.length }}</strong> sub categories
                </span>
            </div>
                    ...
                    <table class="w-full min-w-[640px]">
                        <thead class="bg-slate-50 border-b border-slate-200">
                            <tr>
                                ...
                                <th class="text-left text-xs font-semibold uppercase tracking-wider text-slate-500 px-3 py-3 sm:px-4">Category</th>
                        ...
                        <tbody class="divide-y divide-slate-200">
                            <tr v-for="subCategory in paginatedSubCategories" :key="subCategory.id" class="hover:bg-slate-50 transition">
                                <td class="px-3 py-3 text-sm text-slate-600 sm:px-4">#{{ subCategory.id }}</td>
                                ...
                                <td class="px-3 py-3 sm:px-4">
                                    <code class="text-xs bg-slate-100 text-slate-600 px-2 py-1 rounded">{{ subCategory.slug }}</code>
                                </td>
                                <td class="px-3 py-3 sm:px-4">
                                    <span
                                        :class="[
            ...
            <div v-if="filteredSubCategories.length > 0" class="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div class="text-sm text-slate-500 text-center sm:text-left">
                    ...
                    to 
                    <span class="font-medium text-slate-700">{{ Math.min(currentPage * perPage, filteredSubCategories.length) }}</span> 
                    of 
                    <span class="font-medium text-slate-700">{{ filteredSubCategories.length }}</span> 
                    results
                </div>
            ...
            <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4 shadow-xl">
                <div class="flex items-start gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <p v-if="deleteItem?.tracks?.length > 0" class="text-sm text-amber-600 mt-2">
                ...
                <div class="flex justify-end gap-3 border-t border-slate-200 pt-4">
                    ...
                    <button @click="deleteSubCategory" class="px-4 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 rounded-lg transition">
                        Delete Sub Category
                    </button>
                </div>
            </div>
