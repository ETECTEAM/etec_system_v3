        ...
        <div class="w-full">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-slate-900 dark:text-gray-100 tracking-tight">{{ $t('Lessons') }}</h1>
                        <p class="text-sm text-slate-500 dark:text-gray-400 mt-0.5">{{ $t('Read, create, update, and delete lesson records') }}</p>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-2 gap-3 mb-6">
                <div class="bg-white dark:bg-gray-900 rounded-xl border border-slate-200 dark:border-gray-800 px-4 py-3.5">
                    ...
                    <p class="text-xl font-bold text-slate-900 dark:text-gray-100 mt-1">{{ lessons.length }}</p>
                </div>
                <div class="bg-white dark:bg-gray-900 rounded-xl border border-slate-200 dark:border-gray-800 px-4 py-3.5">
            ...
            <div class="bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-xl p-4 mb-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                <button v-if="hasActiveFilters" @click="resetFilters"
                    class="px-4 py-2 text-sm font-medium text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 border border-blue-200 hover:border-blue-300 dark:border-blue-500/30 dark:hover:border-blue-500/50 rounded-lg transition flex items-center justify-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <!-- Results Count -->
            <div class="mb-3 text-sm text-slate-500 dark:text-gray-400">
                {{ $t('Showing') }} <strong class="text-slate-700 dark:text-gray-200">{{ paginatedLessons.length }}</strong> {{ $t('of') }}
                <strong class="text-slate-700 dark:text-gray-200">{{ filteredLessons.length }}</strong> {{ $t('lessons') }}
            </div>
                    ...
                    <table class="w-full min-w-[640px]">
                        <thead class="bg-slate-50/80 dark:bg-gray-800/60 border-b border-slate-200 dark:border-gray-800">
                            <tr>
                                ...
                                <th class="text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-gray-400 px-4 py-3.5">{{ $t('Course') }}</th>
                                <th class="text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-gray-400 px-4 py-3.5">{{ $t('Order') }}</th>
                                <th class="text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-gray-400 px-4 py-3.5">{{ $t('Duration') }}</th>
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-gray-800">
                            <tr v-for="(lesson, index) in paginatedLessons" :key="lesson.id" class="hover:bg-slate-50/60 dark:hover:bg-gray-800/50 transition-colors">
                                ...
                                <td class="px-4 py-3.5">
                                    ...
                                    <p v-if="lesson.description" class="text-xs text-slate-500 dark:text-gray-400 truncate max-w-xs">{{ lesson.description }}</p>
                                </td>
                                <td class="px-4 py-3.5 text-sm text-slate-600 dark:text-gray-300">{{ lesson.course?.title }}</td>
                                <td class="px-4 py-3.5 text-sm text-slate-600 dark:text-gray-300">{{ lesson.order_number || 0 }}</td>
                                <td class="px-4 py-3.5 text-sm text-slate-600 dark:text-gray-300">{{ lesson.duration || 0 }} {{ $t('min') }}</td>
                                <td class="px-4 py-3.5">
                                    <span class="inline-flex items-center gap-1.5 text-xs font-medium"
                                        :class="lesson.status === 'active' ? 'text-emerald-700 dark:text-emerald-400' : 'text-rose-700 dark:text-rose-400'">
