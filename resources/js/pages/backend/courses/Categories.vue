    ...
    <DashboardLayout>
        <div class="p-4 sm:p-6">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900 dark:text-gray-100">Categories</h1>
                    <p class="text-sm text-slate-500 dark:text-gray-400">Manage course categories</p>
                </div>
                <Link
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
                ...
                <span class="text-sm text-slate-500 whitespace-nowrap dark:text-gray-400">
                    Showing <strong class="text-slate-700 dark:text-gray-200">{{ paginatedCategories.length }}</strong> of
                    <strong class="text-slate-700 dark:text-gray-200">{{ filteredCategories.length }}</strong> categories
                </span>
            </div>
                    ...
                    <table class="w-full min-w-[640px]">
                        <thead class="bg-slate-50 border-b border-slate-200 dark:bg-gray-800 dark:border-gray-800">
                            <tr>
                                ...
                                <th class="text-left text-xs font-semibold uppercase tracking-wider text-slate-500 px-3 py-3 sm:px-4 dark:text-gray-400">Sub Categories</th>
                        ...
                        <tbody class="divide-y divide-slate-200 dark:divide-gray-800">
                            <tr v-for="category in paginatedCategories" :key="category.id" class="hover:bg-slate-50 transition dark:hover:bg-gray-800">
                                <td class="px-3 py-3 text-sm text-slate-600 sm:px-4 dark:text-gray-300">#{{ category.id }}</td>
                                <td class="px-3 py-3 sm:px-4">
                                    <span class="text-sm font-medium text-slate-900 dark:text-gray-100">{{ category.name }}</span>
                                </td>
                                <td class="px-3 py-3 sm:px-4">
                                    <code class="text-xs bg-slate-100 text-slate-600 px-2 py-1 rounded dark:bg-gray-800 dark:text-gray-300">{{ category.slug }}</code>
                                </td>
                                <td class="px-3 py-3 sm:px-4">
                                ...
                                <td class="px-3 py-3 sm:px-4">
                                    <span
                                        :class="[
                                            ...
                                            category.status === 'active' ? 'bg-green-100 text-green-800 dark:bg-green-500/10 dark:text-green-400' : 'bg-red-100 text-red-800 dark:bg-red-500/10 dark:text-red-400'
                                        ]"
                                    >
            ...
            <div v-if="filteredCategories.length > 0" class="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div class="text-sm text-slate-500 text-center sm:text-left dark:text-gray-400">
                    ...
                    to
                    <span class="font-medium text-slate-700 dark:text-gray-200">{{ Math.min(currentPage * perPage, filteredCategories.length) }}</span>
                    of
                    <span class="font-medium text-slate-700 dark:text-gray-200">{{ filteredCategories.length }}</span>
                    results
                </div>
        ...
        <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="showDeleteModal = false">
            <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4 shadow-xl dark:bg-gray-900">
                <div class="flex items-start gap-4 mb-4">
                    ...
                    <div>
                        <h3 class="text-lg font-semibold text-slate-900 dark:text-gray-100">Delete Category</h3>
                        ...
                        <p v-if="deleteItem?.sub_categories?.length > 0" class="text-sm text-amber-600 mt-2 dark:text-amber-400">
                ...
                <div class="flex justify-end gap-3 border-t border-slate-200 pt-4 dark:border-gray-800">
                    ...
                    <button @click="deleteCategory" class="px-4 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 rounded-lg transition dark:bg-red-600 dark:hover:bg-red-500">
                        Delete Category
                    </button>
                </div>
            </div>
