    ...
    <DashboardLayout>
        ...
        <!-- Breadcrumb -->
            <nav class="flex items-center gap-1.5 text-sm text-slate-400 mb-4">
                ...
                <Link href="/dashboard/course/lessons" class="hover:text-slate-600 transition">{{ $t('Lessons') }}</Link>
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
            </svg>
        ...
        <div class="w-full">
            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-6">
                <div class="flex items-center gap-3">
                    <Link href="/dashboard/course/lessons" class="text-slate-500 hover:text-slate-700 transition p-1.5 rounded-lg hover:bg-slate-100 dark:text-gray-400 dark:hover:text-gray-200 dark:hover:bg-gray-800" :title="$t('Back')">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
                        </svg>
                    </Link>
                    <h1 class="text-2xl font-bold text-slate-900 dark:text-gray-100">
                        {{ lesson ? $t('Edit Lesson') : $t('Create Lesson') }}
                    </h1>
                </div>
            ...
            <form @submit.prevent="submit" class="bg-white rounded-lg border border-slate-200 p-4 sm:p-6 shadow-sm dark:bg-gray-900 dark:border-gray-800">
                <div class="space-y-5">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1.5 dark:text-gray-300">
                            {{ $t('Course') }} <span class="text-red-500 dark:text-red-400">*</span>
                        </label>
                        <select
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1.5 dark:text-gray-300">
                            {{ $t('Title') }} <span class="text-red-500 dark:text-red-400">*</span>
                        </label>
                        <input
                            v-model="form.title"
                            type="text"
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            :placeholder="$t('Enter lesson description (optional)')"
                        />
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1.5 dark:text-gray-300">{{ $t('Order Number') }}</label>
                            <input
                                ...
                                placeholder="1"
                            />
                            <p class="mt-1 text-xs text-slate-500 dark:text-gray-400">{{ $t('Order of lesson in the course') }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1.5 dark:text-gray-300">{{ $t('Duration (minutes)') }}</label>
                            <input
                                ...
                                placeholder="0"
                            />
                            <p class="mt-1 text-xs text-slate-500 dark:text-gray-400">{{ $t('Estimated time to complete') }}</p>
                        </div>
                    </div>

                    <!-- Video URL -->
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1.5 dark:text-gray-300">{{ $t('Video URL') }}</label>
                        <input
                    ...
                    <!-- Content -->
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1.5 dark:text-gray-300">{{ $t('Content') }}</label>
                        <textarea
                            v-model="form.content"
                            rows="6"
                            class="w-full rounded-lg border border-slate-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition resize-y dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200 dark:placeholder:text-gray-500"
                            :placeholder="$t('Enter lesson content (optional)')"
                        />
                    </div>
                    ...
                    <div class="flex items-center gap-3 p-3 bg-slate-50 rounded-lg border border-slate-200 dark:bg-gray-800/40 dark:border-gray-700">
                        ...
                        <span class="text-xs text-slate-500 ml-auto dark:text-gray-400">
                            {{ form.status === 'active' ? $t('Lesson will be visible to students') : $t('Lesson will be hidden') }}
                        </span>
                    </div>
                </div>
                ...
                <div class="mt-8 flex flex-col sm:flex-row items-center justify-end gap-3 border-t border-slate-200 pt-6 dark:border-gray-800">
                    ...
                    >
                        {{ form.processing ? $t('Saving...') : (lesson ? $t('Update Lesson') : $t('Create Lesson')) }}
                    </button>
                </div>
            </form>
        </div>
