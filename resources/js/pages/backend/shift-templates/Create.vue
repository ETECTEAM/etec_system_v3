...
const form = useForm({
  ...
  is_active: true,
  day_groups: [
    { days: [1, 2, 3, 4], period: 'daytime', start_time: '09:00', end_time: '17:00' },
  ],
})
...
function submit() {
  ...
  form.transform(() => payload).post('/dashboard/shift-templates', {
    preserveScroll: true,
...
<template>
  <Head :title="$t('Create Shift Template')" />

  <DashboardLayout>
    <section class="space-y-6">
      <Breadcrumbs :items="breadcrumbItems" />
      <PageHero eyebrow="Schedule Management" :title="$t('Create Shift Template')" description="Add a new shift template with day/time blocks." />
        ...
        <form @submit.prevent="submit" class="space-y-6">
          <div class="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <input ref="inputRef" v-model="form.name" type="text" class="w-full rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none transition focus:border-blue-900 focus:ring-2 focus:ring-blue-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:focus:border-blue-500 dark:focus:ring-blue-500/20" :placeholder="$t('e.g. Morning & Afternoon')" />
          ...
          <div class="flex justify-end gap-3 border-t border-slate-200 pt-6 dark:border-gray-800">
            ...
            <button type="submit" :disabled="form.processing" class="rounded-xl bg-blue-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-blue-800 disabled:cursor-not-allowed disabled:opacity-70 dark:bg-blue-600 dark:hover:bg-blue-500">
              {{ form.processing ? $t('Creating...') : $t('Create Template') }}
            </button>
          </div>
        </form>
      </div>
