...
function deleteTemplate(id) {
  if (confirm(t('Are you sure you want to delete this shift template?'))) {
...
<template>
  <DashboardLayout>
    <section class="space-y-6">
      ...
      <PageHero eyebrow="Schedule Management" :title="$t('Shift Templates')" :description="$t('Create and manage instructor shift templates.')" />
      ...
      <div class="bg-white rounded-xl border border-slate-200 shadow-sm dark:bg-gray-900 dark:border-gray-800">
        <div class="border-b border-slate-200 px-6 py-5 dark:border-gray-800">
          <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div class="w-[24%]">
              <input v-model="search" type="text" :placeholder="$t('Search templates...')" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm text-slate-700 outline-none transition focus:border-blue-600 focus:ring-2 focus:ring-blue-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:focus:border-blue-500 dark:focus:ring-blue-500/20" />
            </div>
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="bg-gray-50 border-b border-gray-200 dark:bg-gray-800 dark:border-gray-800">
                <th class="px-6 py-3 text-left text-slate-600 dark:text-gray-300">{{ $t('Name') }}</th>
                <th class="px-6 py-3 text-left text-slate-600 dark:text-gray-300">{{ $t('Code') }}</th>
                <th class="px-6 py-3 text-left text-slate-600 dark:text-gray-300">{{ $t('Type') }}</th>
                <th class="px-6 py-3 text-center text-slate-600 dark:text-gray-300">{{ $t('Blocks') }}</th>
                <th class="px-6 py-3 text-center text-slate-600 dark:text-gray-300">{{ $t('Active') }}</th>
            ...
            <tbody>
              <tr v-for="template in templates.data" :key="template.id" class="border-t border-slate-200 hover:bg-slate-50 transition dark:border-gray-800 dark:hover:bg-gray-800">
                ...
                <td class="px-6 py-4 text-slate-600 dark:text-gray-300">{{ $t(employmentLabel(template.employment_type)) }}</td>
              ...
              <tr v-if="!templates?.data?.length">
                <td colspan="6" class="py-10 text-center text-slate-500 dark:text-gray-400">{{ search ? $t('No results for ":search"', { search }) : $t('No shift templates found.') }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div class="flex flex-col gap-3 border-t border-slate-200 bg-slate-50 px-6 py-4 sm:flex-row sm:items-center sm:justify-between dark:border-gray-800 dark:bg-gray-800/40">
          <p class="text-sm text-slate-500 dark:text-gray-400">{{ $t('Showing :from-:to of :total templates', { from: templates.from ?? 0, to: templates.to ?? 0, total: templates.total ?? 0 }) }}</p>
          <div class="flex flex-wrap gap-2 text-sm">
            <Link v-for="link in templates.links" :key="link.label" :href="link.url || '#'" v-html="paginationLabel(link.label)" class="px-3 py-2 rounded-lg border text-sm transition dark:border-gray-700 dark:text-gray-300"
