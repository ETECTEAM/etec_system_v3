<template>
    <DashboardLayout>
        ...
        <nav class="flex items-center gap-1.5 text-sm text-slate-400 dark:text-gray-500 mb-4">
            ...
            <Link href="/dashboard/times" class="hover:text-slate-600 dark:hover:text-gray-300 transition">{{ $t('Times') }}</Link>
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
            </svg>
            <span class="text-slate-600 dark:text-gray-300 font-medium">{{ $t('Create') }}</span>
        </nav>

        <div class="w-full">
            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-6">
                <div class="flex items-center gap-3">
                    <Link href="/dashboard/times" class="text-slate-500 hover:text-slate-700 transition p-1.5 rounded-lg hover:bg-slate-100 dark:text-gray-400 dark:hover:text-gray-200 dark:hover:bg-gray-800" :title="$t('Back')">
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1.5 dark:text-gray-300">
                            {{ $t('Time Name') }} <span class="text-red-500 dark:text-red-400">*</span>
                        </label>
                        <input
                            ref="inputRef"
                            ...
                            required
                        />
                        <p v-if="form.errors.time_name" class="mt-1.5 text-sm text-red-600 flex items-center gap-1 dark:text-red-400">
