    ...
    <DashboardLayout>
        <div class="w-full">
            ...
            <nav class="flex items-center gap-1.5 text-sm text-slate-400 dark:text-gray-500 mb-4">
                ...
                <span class="text-slate-600 dark:text-gray-300 font-medium">{{ $t('Times') }}</span>
            </nav>

            <PageHero
                eyebrow="Times Management"
                :title="$t('Times')"
                :description="$t('Read, create, update, and delete times records.')"
                class="mb-6"
            />
            ...
            <Card padding="p-0">
                <div class="border-b border-slate-200 px-6 py-5 dark:border-gray-800">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <div class="min-w-0 shrink-0">
                            <p class="text-xs font-semibold uppercase tracking-[0.24em] text-slate-400 dark:text-gray-500">{{ $t('Time Directory') }}</p>
                            <p class="mt-1 text-sm text-slate-500 dark:text-gray-400">{{ $t('Read, create, update, and delete times records.') }}</p>
                        </div>
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead>{{ $t('Time Name') }}</TableHead>
                                <TableHead class="text-right">{{ $t('Actions') }}</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            <TableRow v-for="(time, index) in paginatedTimes" :key="time.id">
                                <TableCell class="text-sm text-slate-500 dark:text-gray-400">
                                    {{ (currentPage - 1) * perPage + index + 1 }}
                                </TableCell>
                                <TableCell>
                                    <span class="text-sm font-medium text-slate-900 dark:text-gray-100">{{ time.time_name }}</span>
                                </TableCell>
                                <TableCell class="text-right">
                                    <div class="flex justify-end gap-2">
                                        ...
                                        <button
                                            ...
                                            :title="$t('Delete')"
                                            :aria-label="$t('Delete time')"
                                            @click="confirmDelete(time)"
                                        >
                            ...
                            <!-- Empty state: no times exist at all -->
                            <TableRow v-if="paginatedTimes.length === 0 && times.length === 0">
                                <TableCell colspan="3" class="px-4 py-16 text-center">
                                    ...
                                    <p class="text-sm font-medium text-slate-600 dark:text-gray-300">{{ $t('No times yet') }}</p>
                                    <p class="text-xs text-slate-400 dark:text-gray-500 mt-1">{{ $t('Create your first time slot to start scheduling classes') }}</p>
                                    <Link href="/dashboard/times/create"
                            ...
                            <TableRow v-else-if="paginatedTimes.length === 0">
                                <TableCell colspan="3" class="px-4 py-16 text-center">
                                    <svg class="w-12 h-12 mx-auto text-slate-300 dark:text-gray-700 mb-3" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div v-if="filteredTimes.length > 0"
                    ...
                    <p class="text-sm text-slate-500 dark:text-gray-400">
                        {{ $t('Showing :from to :to of :total times', { from: rangeStart, to: rangeEnd, total: filteredTimes.length }) }}
                    </p>
        ...
        <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-[2px]"
            ...
            <div class="bg-white dark:bg-gray-900 rounded-2xl p-6 max-w-md w-full mx-4 shadow-xl">
                <div class="flex items-start gap-4 mb-4">
                    ...
                    <div>
                        <h3 class="text-lg font-semibold text-slate-900 dark:text-gray-100">{{ $t('Delete time') }}</h3>
                        <p class="text-sm text-slate-600 dark:text-gray-400 mt-1">
                            {{ $t('Are you sure you want to delete') }} "<span class="font-medium text-slate-900 dark:text-gray-100">{{ deleteItem?.time_name }}</span>"?
...
const filteredTimes = computed(() => {
    ...
    return props.times.filter(time =>
        time.time_name.toLowerCase().includes(search.value.toLowerCase())
    );
});

// Total pages
const totalPages = computed(() => {
    return Math.ceil(filteredTimes.value.length / perPage.value) || 1;
});
...
const paginatedTimes = computed(() => {
    ...
    return filteredTimes.value.slice(start, end);
});
...
const rangeStart = computed(() => {
    if (filteredTimes.value.length === 0) return 0;
    return (currentPage.value - 1) * perPage.value + 1;
});

const rangeEnd = computed(() => {
    return Math.min(currentPage.value * perPage.value, filteredTimes.value.length);
});
...
const confirmDelete = (time) => {
    deleteItem.value = time;
    showDeleteModal.value = true;
};

const deleteTime = () => {
    if (deleteItem.value) {
        router.delete(`/dashboard/times/${deleteItem.value.id}`, {
            onSuccess: () => {
                showDeleteModal.value = false;
                deleteItem.value = null;
            },
            onError: (errors) => {
                alert(errors.message || 'Failed to delete time');
            }
        });
    }
};
