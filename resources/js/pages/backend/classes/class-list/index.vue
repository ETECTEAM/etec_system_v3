...
const handleAction = (action, item) => {
    switch (action) {
        case "view":
            router.visit(
                `/dashboard/class-list/${item.id ?? item.class_list_id}`,
            );
            break;
        case "edit":
            router.visit(
                `/dashboard/class-list/${item.id ?? item.class_list_id}/edit`,
            );
            break;
        case "delete":
            deleteItem(item);
      ...
      <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Card class="overflow-hidden">
                <div class="overflow-x-auto">
                    <table
                        class="min-w-full text-left text-sm text-slate-700 dark:text-gray-300"
                    >
                        <thead
                            class="bg-slate-100 text-slate-700 uppercase text-xs tracking-wider dark:bg-gray-800 dark:text-gray-300"
                        >
                            <tr>
                        ...
                        <tbody>
                            <tr
                                ...
                                :key="item.id ?? item.class_list_id"
                                class="border-t border-slate-200 hover:bg-slate-50 dark:border-gray-800 dark:hover:bg-gray-800"
                            >
                                <td class="px-4 py-4">
                                    <span
                                        class="inline-flex rounded-full bg-blue-800 px-3 py-1 text-xs font-semibold text-white dark:bg-blue-600"
                                        >ID:
                                        {{
                                            item.id ?? item.class_list_id
                                        }}</span
                                    >
                                </td>
                                <td class="px-4 py-4">
                                ...
                                <td class="px-4 py-4">
                                    ...
                                    >
                                        Lesson:
                                        <span
                                            class="rounded-full bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-700 dark:bg-gray-800 dark:text-gray-300"
                                            >{{
                                                item.lesson?.title ||
                                                "No lesson"
                                            }}</span
                                ...
                                <td class="px-4 py-4">
                                    <div
                                        class="font-semibold text-slate-900 dark:text-gray-100"
                                    >
                                        {{ item.term?.term_name || "No term" }}
                                    </div>
                                    <div
                                        class="mt-1 text-xs text-slate-500 dark:text-gray-400"
                                    >
                                        {{ item.time?.time_name || "No time" }}
                                    </div>
                                </td>
                                <td class="px-4 py-4">
                                    <div
                                    ...
                                    >
                                        {{
                                            item.building?.name || "No building"
                                        }}
                                    </div>
                                    <div
                                        class="mt-1 text-xs text-slate-500 dark:text-gray-400"
                                    >
                                        {{ item.floor?.name || "No floor" }} ·
                                        {{
                                            item.room?.room_number || "No room"
                                        }}
                                    </div>
                                </td>
                                <td
                                    class="px-4 py-4 font-semibold text-slate-900 dark:text-gray-100"
                                >
                            ...
                            <tr v-if="filteredData.length === 0">
                                <td
                                    colspan="9"
                                    class="px-4 py-12 text-center text-slate-500 dark:text-gray-400"
                                >
                                    No classes found.
                                </td>
                            </tr>
