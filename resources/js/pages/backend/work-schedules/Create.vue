...
function submit() {
  ...
  form.transform(() => payload).post('/dashboard/work-schedules', {
    preserveScroll: true,
  })
}
...
<template>
  <Head :title="$t('Create Work Schedule')" />
        ...
        <form @submit.prevent="submit" class="space-y-6">
          <div class="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <input ref="inputRef" v-model="form.name" type="text" class="w-full rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none transition focus:border-blue-900 focus:ring-2 focus:ring-blue-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:focus:border-blue-500 dark:focus:ring-blue-500/20" :placeholder="$t('e.g. Morning & Evening')" />
          ...
          <div class="flex justify-end gap-3 border-t border-slate-200 pt-6 dark:border-gray-800">
            ...
            <button type="submit" :disabled="form.processing || totalSelected === 0" class="rounded-xl bg-blue-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-blue-800 disabled:cursor-not-allowed disabled:opacity-70 dark:bg-blue-600 dark:hover:bg-blue-500">
