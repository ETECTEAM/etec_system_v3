  ...
  <DashboardLayout>
    <section class="space-y-6">
      ...
      <PageHero
        ...
        description="Create featured news with publishing details and image galleries."
      />
      ...
      <form class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_420px]" @submit.prevent="submit">
        <div class="space-y-6">
          <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <h2 class="text-lg font-bold text-slate-900 dark:text-gray-100">News Details</h2>
            <div class="mt-5 space-y-5">
              ...
              <div v-if="isEditing" class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm dark:border-gray-800 dark:bg-gray-800/50">
                <span class="font-semibold text-slate-700 dark:text-gray-300">Generated Slug</span>
                <span class="mt-1 block text-slate-500 dark:text-gray-400">/news/{{ props.newsData.slug }}</span>
              </div>

              <div class="grid gap-5 md:grid-cols-2">
                <div>
                  <label class="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-gray-300">Published Date</label>
              ...
              <div>
                <label class="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-gray-300">Short Description</label>
              ...
              <div class="grid gap-3 md:grid-cols-2">
                <label class="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 dark:border-gray-800 dark:bg-gray-800/50">
                  <span class="block text-sm font-semibold text-slate-800 dark:text-gray-100">Published</span>
          ...
          <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <h2 class="text-lg font-bold text-slate-900 dark:text-gray-100">News Images</h2>
            <div class="mt-5">
              ...
              <p class="mt-1 text-xs font-semibold text-slate-500 dark:text-gray-400">{{ imageCount }}/{{ maxImages }} images used. JPEG, PNG, WebP up to 4MB each.</p>
              ...
              <div v-if="existingImages.length || imagePreviews.length" class="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                <div v-for="image in existingImages" :key="image.id" class="overflow-hidden rounded-xl border border-slate-200 bg-slate-50 dark:border-gray-700 dark:bg-gray-800">
                  <img :src="image.image_url" alt="News image" class="aspect-video w-full object-cover" />
                  <div class="flex items-center justify-between gap-3 p-3">
                    <button type="button" class="rounded-lg px-3 py-1.5 text-xs font-bold" :class="form.image_states[image.id] ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600 dark:bg-gray-700 dark:text-gray-300'" @click="toggleImage(image)">
                      {{ form.image_states[image.id] ? "Active" : "Inactive" }}
                    </button>
                    <button type="button" class="text-xs font-bold text-rose-600 hover:text-rose-700" @click="removeImage(image)">Remove</button>
                  </div>
                </div>
                ...
                <div v-for="(url, index) in imagePreviews" :key="url" class="overflow-hidden rounded-xl border border-blue-200 bg-blue-50 dark:border-blue-500/20 dark:bg-blue-500/10">
                  <img :src="url" alt="New news image" class="aspect-video w-full object-cover" />
                  <p class="p-3 text-xs font-bold text-blue-700 dark:text-blue-300">New image {{ index + 1 }} - Active after save</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="flex flex-wrap justify-end gap-3">
            <Link href="/dashboard/website/news" class="rounded-xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 dark:border-gray-700 dark:text-gray-200">Cancel</Link>
            <button type="submit" :disabled="form.processing" class="rounded-xl bg-blue-600 px-6 py-3 text-sm font-semibold text-white transition hover:bg-blue-700 disabled:opacity-50">
              {{ form.processing ? "Saving..." : isEditing ? "Update News" : "Create News" }}
            </button>
          </div>
        </div>
        ...
        <aside class="space-y-4">
          <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <p class="mb-3 text-sm font-bold text-slate-900 dark:text-gray-100">News Preview</p>
            ...
            <div class="mt-4 space-y-2">
              ...
              <h3 class="text-lg font-bold text-slate-900 dark:text-gray-100">{{ form.title || "News title" }}</h3>
              <p class="text-sm leading-6 text-slate-600 dark:text-gray-300">{{ form.excerpt || form.description || "News description" }}</p>
            </div>
          </div>
        </aside>
      </form>
