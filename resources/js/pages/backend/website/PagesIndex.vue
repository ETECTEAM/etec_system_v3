    ...
    <section class="space-y-6">
      ...
      <div class="overflow-hidden rounded-[26px] border border-slate-200 bg-white shadow-[0_18px_45px_rgba(15,23,42,0.08)] dark:border-gray-800 dark:bg-gray-900 dark:shadow-[0_18px_45px_rgba(15,23,42,0.28)]">
        <div class="flex flex-col gap-5 border-b border-slate-200 px-6 py-5 lg:flex-row lg:items-start lg:justify-between dark:border-gray-800">
          <div>
            <p class="text-xs font-bold uppercase tracking-[0.32em] text-slate-500 dark:text-slate-400">{{ $t('Page Directory') }}</p>
            <p class="mt-2 text-base text-slate-600 dark:text-slate-300">{{ $t('Read, create, update, and manage public website pages in one place.') }}</p>
          </div>
        ...
        <div class="overflow-x-auto">
          <table class="w-full min-w-[1040px] text-sm">
            ...
            <tbody>
              <tr v-for="(page, index) in pages.data" :key="page.id" class="border-b border-slate-100 text-slate-700 transition hover:bg-slate-50 dark:border-gray-800 dark:text-slate-200 dark:hover:bg-gray-800/60">
                <td class="px-6 py-4 text-slate-500 dark:text-slate-400">{{ rowNumber(index) }}</td>
                <td class="px-6 py-4 font-semibold text-slate-900 dark:text-white">{{ page.title }}</td>
                <td class="px-6 py-4">
                  <span class="inline-flex rounded-lg bg-slate-100 px-3 py-1 font-mono text-xs text-slate-700 dark:bg-gray-800 dark:text-slate-300">/{{ page.slug }}</span>
                </td>
                <td class="px-6 py-4 text-slate-600 dark:text-slate-300">{{ page.connected_menu || $t('Not connected') }}</td>
                <td class="px-6 py-4">
                  <span class="rounded-full px-3 py-1 text-xs font-semibold" :class="page.hero_status === 'Hero Enabled' ? 'bg-emerald-500/15 text-emerald-700 dark:text-emerald-300' : 'bg-slate-100 text-slate-600 dark:bg-gray-800 dark:text-slate-300'">
                    {{ $t(page.hero_status) }}
                  </span>
                </td>
                <td class="px-6 py-4">
                  <button
                    ...
                    :class="page.is_active ? 'bg-emerald-500/15 text-emerald-700 hover:bg-emerald-500/25 dark:text-emerald-300' : 'bg-rose-500/15 text-rose-700 hover:bg-rose-500/25 dark:text-rose-300'"
                    @click="togglePage(page)"
                  >
                    {{ page.is_active ? $t('Active') : $t('Inactive') }}
                  </button>
                </td>
                <td class="px-6 py-4 text-slate-500 dark:text-slate-400">{{ page.created_at }}</td>
                <td class="px-6 py-4">
