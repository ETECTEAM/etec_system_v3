    ...
    <section class="space-y-6">
      ...
      <div class="grid gap-6 lg:grid-cols-[1fr_320px]">
        <article class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
          <div class="prose max-w-none dark:prose-invert" v-html="pageData.content || `<p>${$t('No content yet.')}</p>`"></div>
        </article>
        <aside class="space-y-4">
          <div class="rounded-xl border border-slate-200 bg-white p-5 text-sm shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <p class="font-bold text-slate-900 dark:text-gray-100">{{ $t('Page Details') }}</p>
            <dl class="mt-4 space-y-3 text-slate-600 dark:text-gray-300">
              <div class="flex justify-between gap-3"><dt>{{ $t('Status') }}</dt><dd class="font-semibold">{{ $t(pageData.is_active ? "Active" : "Inactive") }}</dd></div>
              <div class="flex justify-between gap-3"><dt>{{ $t('Hero') }}</dt><dd class="font-semibold">{{ $t(pageData.hero?.is_active ? "Enabled" : "Disabled") }}</dd></div>
              <div class="flex justify-between gap-3"><dt>{{ $t('Menus') }}</dt><dd class="font-semibold">{{ pageData.menus?.length ?? 0 }}</dd></div>
            </dl>
          </div>
