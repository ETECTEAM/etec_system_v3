    ...
    <section class="space-y-6">
      ...
      <div class="overflow-hidden rounded-[26px] border border-slate-800 bg-slate-900 shadow-[0_18px_45px_rgba(15,23,42,0.28)]">
        <div class="flex flex-col gap-5 border-b border-slate-800 px-6 py-5 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p class="text-xs font-bold uppercase tracking-[0.32em] text-slate-400">{{ $t('News Directory') }}</p>
            <p class="mt-2 text-base text-slate-300">{{ $t('Read, create, update, and publish website news records.') }}</p>
          </div>
        ...
        <div class="overflow-x-auto">
          <table class="w-full min-w-[1160px] text-sm">
            ...
            <tbody>
              <tr v-for="(item, index) in news.data" :key="item.id" class="border-b border-slate-800 text-slate-200 transition hover:bg-slate-800/55">
                <td class="px-6 py-4 text-slate-400">{{ rowNumber(index) }}</td>
                <td class="px-6 py-4">
                  <div class="flex min-w-0 items-center gap-4">
                    <div class="h-16 w-24 flex-shrink-0 overflow-hidden rounded-xl bg-slate-800">
                    ...
                    <div class="min-w-0">
                      ...
                      <p class="mt-1"><span class="inline-flex rounded-lg bg-slate-800 px-3 py-1 font-mono text-xs text-slate-300">/{{ item.slug }}</span></p>
                      <p class="mt-2 line-clamp-2 max-w-md text-xs text-slate-400">{{ item.excerpt || item.description || $t('No description') }}</p>
                    </div>
                  </div>
                </td>
                <td class="px-6 py-4 text-slate-300">{{ item.author || $t('Not set') }}</td>
                <td class="px-6 py-4 text-slate-300">{{ item.published_at || $t('Not set') }}</td>
                <td class="px-6 py-4 text-slate-300">{{ item.images_count }} {{ $t('images') }}</td>
                <td class="px-6 py-4">
                  <span class="inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-semibold" :class="item.is_featured ? 'bg-amber-500/15 text-amber-300' : 'bg-slate-800 text-slate-300'">
                ...
                <td class="px-6 py-4">
                  <button
                    ...
                    :class="item.is_active ? 'bg-emerald-500/15 text-emerald-300 hover:bg-emerald-500/25' : 'bg-rose-500/15 text-rose-300 hover:bg-rose-500/25'"
