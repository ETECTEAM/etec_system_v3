    ...
    <section class="space-y-6">
      ...
      <div class="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
          ...
          <table class="w-full min-w-[1080px] text-sm">
            <thead>
              <tr class="border-b border-slate-200 bg-slate-50 text-left text-slate-600 dark:border-gray-800 dark:bg-gray-800 dark:text-gray-300">
                <th class="px-6 py-3">{{ $t('News') }}</th>
                <th class="px-6 py-3">{{ $t('Author') }}</th>
                <th class="px-6 py-3">{{ $t('Published') }}</th>
                <th class="px-6 py-3">{{ $t('Images') }}</th>
                ...
                <td class="px-6 py-4">
                  <div class="flex min-w-0 items-center gap-4">
                    ...
                    <div class="min-w-0">
                      ...
                      <p class="mt-1 line-clamp-2 max-w-md text-xs text-slate-500 dark:text-gray-400">{{ item.excerpt || item.description || $t('No description') }}</p>
                    </div>
                  </div>
                </td>
                <td class="px-6 py-4 text-slate-600 dark:text-gray-300">{{ item.author || $t('Not set') }}</td>
                <td class="px-6 py-4 text-slate-600 dark:text-gray-300">{{ item.published_at || $t('Not set') }}</td>
        ...
        <div class="flex flex-col gap-3 border-t border-slate-200 bg-slate-50 px-6 py-4 sm:flex-row sm:items-center sm:justify-between dark:border-gray-800 dark:bg-gray-800/40">
          <p class="text-sm text-slate-500 dark:text-gray-400">{{ $t('Showing :from-:to of :total news', { from: news.from ?? 0, to: news.to ?? 0, total: news.total ?? 0 }) }}</p>
