...
function removeLogo() {
  if (!window.confirm(t("Remove the current school logo?"))) return;
...
<template>
  <DashboardLayout>
    <section class="space-y-6">
      ...
      <PageHero eyebrow="Website Management" :title="$t('School Settings')" :description="$t('Manage the public school name and logo from one place.')" />
      ...
      <form class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900" @submit.prevent="submit">
        <div class="grid gap-6 lg:grid-cols-[1fr_320px]">
          <div class="space-y-5">
            <div>
              <label class="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-gray-300">{{ $t('School Name') }}</label>
              <input
                ...
                :placeholder="$t('Engineer of Technology and Electronic Center')"
              />
              <p v-if="form.errors.school_name" class="mt-1 text-sm text-rose-600">{{ form.errors.school_name }}</p>
            </div>

            <div>
              <label class="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-gray-300">{{ $t('School Logo') }}</label>
              <input
                type="file"
          ...
          <div class="rounded-xl border border-slate-200 bg-slate-50 p-5 dark:border-gray-800 dark:bg-gray-800/40">
            <p class="mb-4 text-sm font-semibold text-slate-700 dark:text-gray-200">{{ $t('Brand Preview') }}</p>
            <div class="flex items-center gap-3 rounded-xl bg-white p-4 shadow-sm dark:bg-gray-900">
              <div class="flex h-14 w-14 items-center justify-center overflow-hidden rounded-xl bg-blue-100 text-lg font-bold text-blue-700 dark:bg-blue-500/10 dark:text-blue-300">
                ...
                <span v-else>{{ $t('ET') }}</span>
              </div>
              <div>
                <p class="text-sm font-bold text-slate-900 dark:text-gray-100">{{ form.school_name || $t('School Name') }}</p>
                <p class="text-xs text-slate-500 dark:text-gray-400">{{ $t('Public header, footer, and admin header') }}</p>
              </div>
            </div>
          </div>
        </div>
