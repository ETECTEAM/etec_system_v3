...
<template>
  ...
  <form class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900" @submit.prevent="savePermissions">
    <div class="flex flex-col gap-3 border-b border-slate-200 p-5 sm:flex-row sm:items-center sm:justify-between dark:border-gray-800">
      <div>
        <h2 class="text-base font-bold text-slate-900 dark:text-gray-100">{{ $t('User Permissions Matrix') }}</h2>
        <p class="mt-1 text-sm text-slate-600 dark:text-gray-400">{{ $t('Blue checks are direct permissions. Pale checks already come from the user\'s role.') }}</p>
      </div>
      <div class="flex flex-wrap items-center gap-3">
        <div class="flex gap-3 text-sm font-semibold">
          <span class="text-slate-500 dark:text-gray-400">{{ $t('Role :count', { count: selectedRoleCount }) }}</span>
          <span class="text-blue-700 dark:text-blue-400">{{ $t('Direct :count', { count: selectedDirectCount }) }}</span>
        </div>
    ...
    <div class="flex flex-col gap-3 border-b border-slate-200 px-5 py-3 sm:flex-row sm:items-center sm:justify-end dark:border-gray-800">
      ...
      <div class="relative sm:max-w-xs">
        ...
        <input v-model="moduleSearch" type="text" :placeholder="$t('Search...')" class="w-full rounded-lg border border-slate-300 py-2 pl-9 pr-3 text-sm text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-blue-600 focus:ring-2 focus:ring-blue-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:placeholder:text-gray-500 dark:focus:border-blue-500 dark:focus:ring-blue-500/20">
      </div>
    </div>
      ...
      <table class="min-w-full divide-y divide-slate-200 text-sm dark:divide-gray-800">
        ...
        <tbody class="divide-y divide-slate-100 bg-white dark:divide-gray-800 dark:bg-gray-900">
          <tr v-for="resource in paginatedResources" :key="resource" class="hover:bg-slate-50 dark:hover:bg-gray-800/60">
            <td class="sticky left-0 z-10 whitespace-nowrap bg-white px-5 py-4 font-semibold capitalize text-slate-800 min-w-[240px] w-[260px] dark:bg-gray-900 dark:text-gray-200">{{ $t(resource.replaceAll('_', ' ')) }}</td>
          ...
          <tr v-if="paginatedResources.length === 0">
            <td :colspan="actions.length + 1" class="px-5 py-10 text-center text-slate-500 dark:text-gray-400">{{ $t('No modules found.') }}</td>
          </tr>
        </tbody>
      </table>
    </div>
