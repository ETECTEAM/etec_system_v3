...
<template>
  <aside class="self-start rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
    <div class="flex items-center justify-between px-2 pb-3">
      <h2 class="text-base font-bold text-slate-900 dark:text-gray-100">{{ $t('Users') }}</h2>
    ...
    <div class="px-2 pb-3">
      <div class="relative">
        ...
        <input v-model="userSearch" type="text" :placeholder="$t('Search...')" class="w-full rounded-lg border border-slate-300 py-2 pl-9 pr-3 text-sm text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-blue-600 focus:ring-2 focus:ring-blue-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:placeholder:text-gray-500 dark:focus:border-blue-500 dark:focus:ring-blue-500/20">
      </div>
    </div>

    <div class="max-h-[640px] space-y-2 overflow-y-auto pr-1">
      <p v-if="filteredUserSummaries.length === 0" class="px-2 py-6 text-center text-sm text-slate-500 dark:text-gray-400">{{ $t('No users found.') }}</p>
