...
const primaryRole = computed(() => roles.value[0] ?? 'guest')
</script>
    ...
    <section class="space-y-6 p-4 sm:p-6">
      ...
      <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p class="mt-3 text-lg font-bold text-slate-900">{{ user?.email ?? 'Guest session' }}</p>
          <p class="mt-2 text-sm text-slate-600">Authenticated email for the current dashboard session.</p>
        </article>

        <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <p class="text-xs font-semibold uppercase tracking-[0.25em] text-slate-500">Primary Role</p>
          <p class="mt-3 text-lg font-bold capitalize text-slate-900">{{ primaryRole.replace('_', ' ') }}</p>
          <p class="mt-2 text-sm text-slate-600">Permissions and navigation can be tailored from this role.</p>
        </article>

        <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <p class="text-xs font-semibold uppercase tracking-[0.25em] text-slate-500">Status</p>
          <p class="mt-3 text-lg font-bold text-emerald-600">{{ user ? 'Logged in' : 'Signed out' }}</p>
          <p class="mt-2 text-sm text-slate-600">Super admins now land here right after a successful login.</p>
        </article>
      </div>
    </section>
  </DashboardLayout>
